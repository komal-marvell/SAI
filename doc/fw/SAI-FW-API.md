...
Switch level read only attribute SAI_SWITCH_ATTR_MAX_FW_CORES provides number of cores supported. Cores are idenitifed using linear name space for e.x. 4 cores are identified as Core ID of value 0, 1, 2, 3 and 4.

SAI Adapter will map the SAI core id name space to its own internal name space.
...
#### 2.1.3 Existing Switch Level Firmware Attributes
There exist a switch level infra structure using SAI_SWITCH_ATTR_FIRMWARE_DOWNLOAD_BROADCAST, SAI_SWITCH_ATTR_FIRMWARE_LOAD_METHOD, SAI_SWITCH_ATTR_FIRMWARE_LOAD_TYPE and so on, attributes. This infra structure is mainlu defined to load serdes or other PHY level firmware.

This infra structure is still applicable to serdes firmware as the requirements such as broadcast init, stop etc are very specific to that domain.

New sai_fw_t is defined mainly to support switch pipeline related applications.

These two methods since aplicable to different domains can coexist and can be enabled at the same time.
...
sai_query_attribute_enum_values_capability() API can be used to find out what admin states are support for a given firmware object.
